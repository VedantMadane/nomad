...
//
// This could be inline within the NewTaskRunner function, but having a
// separate function makes testing easier.
func (tr *TaskRunner) setHookStatsHandler(ns string) {
	if tr.clientConfig.DisableAllocationHookMetrics {
		tr.hookStatsHandler = hookstats.NewNoOpHandler()
	} else {
		labels := tr.clientBaseLabels
		labels = append(labels, metrics.Label{Name: "namespace", Value: ns})
		tr.hookStatsHandler = hookstats.NewHandler(labels, "task_hook")
	}
}
