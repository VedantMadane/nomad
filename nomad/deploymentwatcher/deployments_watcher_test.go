...
// logRecorder is borrowed from hostvolumemanager and added to the TestDeploymentWatcher
// so we can assert that stdout/stderr appear in logs
...
func TestWatcher_PauseDeployment_IgnoreProgressDeadline(t *testing.T) {
	...
	time.Sleep(time.Until(state.RequireProgressBy))
	...
	modifiedTime := time.Unix(0, d.ModifyTime)
